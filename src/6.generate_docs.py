...
# LLM 配置（统一 request_format/base_url/api_key + summary 模型）
LLM_API_KEY = os.getenv("LLM_API_KEY")
LLM_MODEL = os.getenv("LLM_SUMMARY_MODEL", "gpt-4.1-mini")
LLM_CLIENT = None
if LLM_API_KEY:
    LLM_CLIENT = ClientFactory.from_config(
        {
...
def generate_deep_summary(md_file_path: str, txt_file_path: str, max_retries: int = 3) -> str | None:
    if LLM_CLIENT is None:
        log("[WARN] 未配置 LLM_API_KEY，跳过精读总结。")
        return None
